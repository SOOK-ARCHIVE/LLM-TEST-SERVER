...
app.use(
  cors({
    origin: "https://sook-archive.github.io", // 여러분들 거
    methods: ["POST"],
...
app.post("/", async (req, res) => {
  ...
  const prompt = await callAI({
    ...
    // text,
    text: `${text}의 위치나 스타일 정보를 바탕으로 술집(맥주 펍, 칵테일바, 와인바, 음식점 등등) 술집 추천에 어울리는 AI 이미지 생성을 위한 200자 이내의 영어 프롬프트를 작성해줘`,
  ...
  const prompt2 = await callAI({
    ...
    model: GROQ_LLAMA_MODEL,
    // text,
    text: `${text}를 바탕으로 내가 원하는 스타일의 술집 추천(칵테일바, 와인바, 음식점 등)에 어울리는 설명 생성을 위한 200자 이내의 한글 프롬프트를 작성해줘`,
  }).then((res) => res.choices[0].message.content);
  console.log(prompt2);
  // 3-2. 그거에서 프롬프트만 추출
  // mixtral-8x7b-32768 (groq)
  ...
  const desc = await callAI({
    ...
    text: `${promptJSON2}를 기반으로 마크다운 문법 없이 평문으로 작성해주고 한글 결과물로 출력해줘. 엔터로 줄바꿈을 넣어줘.`,
    max_tokens: 2048,
